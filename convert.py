...
def main(ann_dir='Data', out_dir='output', target_recipe=None):
    # load src csv files
    dfs = []
    for f in tqdm.tqdm(glob.glob(os.path.join(ann_dir, 'Plans/*/*.txt'))):
        df = convert_csv(f, target_recipe=target_recipe)
        if df is None:
            continue
        dfs.append(df)
    df = pd.concat(dfs)
    ...
    # csv output prefix
    suffix = f'_{target_recipe}' if target_recipe else ''
    name = f'cmu_mmac{suffix}'
    print(name)

    # write out csvs/pkl files
    os.makedirs(out_dir, exist_ok=True)
    df.to_csv(os.path.join(out_dir, f'{name}_retrieval_test.csv'))
    df.to_pickle(os.path.join(out_dir, f'{name}_retrieval_test.pkl'))

    sentence_df = df[['narration']].reset_index().groupby('narration').first().reset_index().set_index('narration_id')
    sentence_df.to_csv(os.path.join(out_dir, f'{name}_retrieval_test_sentence.csv'))
    sentence_df.to_pickle(os.path.join(out_dir, f'{name}_retrieval_test_sentence.pkl'))
    print(df)

def convert_csv(csv_file, eaf_file=None, fps=30, target_recipe=None):
    # read csv
